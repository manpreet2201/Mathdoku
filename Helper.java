...
public class Helper {
	...
	// for group should not be in same row or column as it can be diagonally(in add
	// and mult)

	static ArrayList<ArrayList<Integer>> filter_solutions(ArrayList<ArrayList<Integer>> all_solutions,
			ArrayList<int[]> cells) {
		...
		for (ArrayList<Integer> solution : all_solutions) {
			...
			for (int i = 0; i < cells.size(); i++) {
				for (int j = i + 1; j < cells.size(); j++) {
					// if all cells of group are in same row or column as we cannot repeat value
					// then
			...
			// if values not repeated add soln or values repeated are not in same row or
			// column
			if (selected)
	...
	// this method returns a arraylist of arraylist of different combinations to
	// form a group in addition
	static ArrayList<ArrayList<Integer>> sum_combinations(int target, int num_steps, ArrayList<Integer> array) {
		// arraylist to return
		ArrayList<ArrayList<Integer>> res = new ArrayList<ArrayList<Integer>>();

		if (num_steps == 1) {
			// when no of steps is 1 and target left is in array add that no to res
			// arraylist
	...
	static ArrayList<ArrayList<Integer>> mult_combinations(int target, int num_steps, ArrayList<Integer> array) {
		...
		for (Integer i : array) {
			...
			if (target % i != 0)
				continue;
			// calling mult_combinations recursively dividing target with i and subtracting
			// 1 from numsteps
